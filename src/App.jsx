...
function parseRoute() {
  ...
  if (root === 'created' && eventId) return { mode: 'created', eventId, token }
...
function EventCreatePage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      move(`/created/${created.eventId}?adminToken=${created.adminToken}&participantToken=${created.participantToken}`)
    } catch (err) {
...
function CreatedPage({ eventId }) {
  const params = new URLSearchParams(window.location.search)
  const adminToken = params.get('adminToken') || ''
  const participantToken = params.get('participantToken') || ''
  const adminUrl = `${window.location.origin}/admin/${eventId}?token=${adminToken}`
  const joinUrl = `${window.location.origin}/join/${eventId}?token=${participantToken}`

  return (
    <main className="container">
      <section className="card">
        <h1>イベント作成完了</h1>
        <p>参加者には下のURLを共有してください。</p>
        <a href={joinUrl}>{joinUrl}</a>
        <p className="sub">幹事用URL（共有しないでください）</p>
        <a href={adminUrl}>{adminUrl}</a>
        <button onClick={() => move(`/admin/${eventId}?token=${adminToken}`)}>管理画面へ進む</button>
      </section>
    </main>
  )
}
...
export default function App() {
  ...
  if (route.mode === 'created') return <CreatedPage eventId={route.eventId} />
